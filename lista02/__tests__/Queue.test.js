...
describe('Queue', () => {
    ...
    describe('constructor', () => {
        test('deve criar uma fila vazia com tamanho padrão 5 quando nenhum tamanho é especificado', () => {
            const defaultQueue = new Queue();
            expect(defaultQueue.queue.length).toBe(0);
            expect(defaultQueue.limit).toBe(5);
        });
    ...
    describe('size', () => {
        test('deve retornar o tamanho correto da fila', () => {
            queue.enqueue(1);
            queue.enqueue(2);
            // Corrigindo a expectativa assumindo que deveria ser this.queue.length
